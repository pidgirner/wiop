...
- Реальная AI-генерация на backend (OpenRouter) с сохранением истории в таблицу `generations`.
...
- `OPENROUTER_DEFAULT_API_KEY` и `OPENROUTER_DEFAULT_MODEL`
- `OPENROUTER_API_BASE` (опционально, по умолчанию `https://openrouter.ai/api/v1`)
- `OPENROUTER_SITE_URL` и `OPENROUTER_APP_NAME` (опционально, заголовки OpenRouter)
- `OPENROUTER_TEMPERATURE` (опционально, по умолчанию `0.7`)
- `OPENROUTER_MAX_TOKENS` (опционально, по умолчанию `900`)
- `ADMIN_EMAIL`
- `ADMIN_PASSWORD`

### OpenRouter по разделам (разные ключи и модели)

Для каждого типа генерации можно задать свою пару `API key + model`:

- `text`: `OPENROUTER_TEXT_API_KEY`, `OPENROUTER_TEXT_MODEL`
- `image`: `OPENROUTER_IMAGE_API_KEY`, `OPENROUTER_IMAGE_MODEL`
- `video`: `OPENROUTER_VIDEO_API_KEY`, `OPENROUTER_VIDEO_MODEL`
- `audio`: `OPENROUTER_AUDIO_API_KEY`, `OPENROUTER_AUDIO_MODEL`
- `post`: `OPENROUTER_POST_API_KEY`, `OPENROUTER_POST_MODEL`

Логика fallback:

1. Если для типа заполнены `OPENROUTER_<TYPE>_API_KEY` и `OPENROUTER_<TYPE>_MODEL`, используется именно они.
2. Иначе используются `OPENROUTER_DEFAULT_API_KEY` и `OPENROUTER_DEFAULT_MODEL`.
...
2. В Project Settings -> Environment Variables добавьте:
   ...
   - `OPENROUTER_*_API_KEY` и `OPENROUTER_*_MODEL` (если используете разные модели по разделам)
   - `ADMIN_EMAIL`
   - `ADMIN_PASSWORD`
